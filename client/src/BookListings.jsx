...
function BookListings() {
  ...
  async function handleDeleteConfirmed() {
    ...
    try {
      const baseUrl = getApiBaseUrl();
      const res = await fetch(`${baseUrl}/BookListings/${deleteTarget.listing_id}`, { method: "DELETE" });
      if (!res.ok) throw new Error("Failed to delete listing");

      setBooks((prev) =>
        prev.filter((item) => !(item._type === "book" && item.listing_id === deleteTarget.listing_id))
      );
                ...
                return (
                  <div key={book.listing_id} className="book-card">
                    ...
                    <div className="book-card-body">
                      ...
                      {!isNotes && (
                        <div className="badge-status">{book.status || "Available"}</div>
                      )}
