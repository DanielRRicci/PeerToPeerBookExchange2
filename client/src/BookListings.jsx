...
function getStatusStyle(status) {
  switch ((status || "").toLowerCase()) {
    case "active":
      return { bg: "#f0fdf4", text: "#15803d", border: "#86efac", dot: "#22c55e", label: "Active" };
    case "pending":
      return { bg: "#fefce8", text: "#854d0e", border: "#fde047", dot: "#eab308", label: "Pending Review" };
    case "sold":
      return { bg: "#fef2f2", text: "#b91c1c", border: "#fca5a5", dot: "#ef4444", label: "Sold" };
    case "removed":
      return { bg: "#f5f5f5", text: "#777",    border: "#ddd",    dot: "#aaa",    label: "Removed" };
    case "under review":
      return { bg: "#fff7ed", text: "#c2410c", border: "#fdba74", dot: "#f97316", label: "Under Review" };
    default:
      return { bg: "#f0fdf4", text: "#15803d", border: "#86efac", dot: "#22c55e", label: status || "Active" };
  }
}
    ...
    <div className="modal-overlay" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="modal-card" style={{ maxWidth: 420 }}>
        <div className="modal-header">
          ...
          <div style={{ color: "rgba(255,189,0,0.55)", fontSize: 12, marginTop: 4 }}>
            {listing.title}
          </div>
        </div>
...
function BookListings() {
  ...
  async function handleSaveEdit() {
    try {
      ...
      const safeStatus = allowedStatuses.includes(editData.status)
        ? editData.status
        : "Active";
    ...
    <>
      ...
      {isAdmin && (
        <div className="admin-banner">
          ...
          <button className="admin-dashboard-link" onClick={() => navigate("/admin")}>
            Admin Dashboard →
          </button>
        </div>
      )}
                          ...
                          {isNotes ? (
                            ...
                            <>
                              ...
                              {isAdmin && (
                                <>
                                  {book.status === "Pending" && (
                                    <button className="btn-approve" onClick={() => handleApprove(book.listing_id)}>
                                      ✓ Approve
                                    </button>
                                  )}
                                  <button className="btn-admin" onClick={() => setAdminTarget(book)}>
                                    ⚙ Status
                                  </button>
                                  <button className="btn-danger" onClick={() => setDeleteTarget(book)}>Remove</button>
                                </>
                              )}
                            </>
                          )}
