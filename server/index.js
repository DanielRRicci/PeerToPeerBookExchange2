...
async function sendVerificationEmail(email, code) {
  ...
  if (!transporter) {
    throw new Error('Missing SMTP configuration. Set SMTP_HOST, SMTP_PORT, SMTP_USER, SMTP_PASS in backend/.env.');
  }
  const fromAddress = process.env.SMTP_FROM || 'Peer-To-Peer Book Exchange <no-reply@p2pbookexchange.com>';
  const toAddress = process.env.SMTP_TO_OVERRIDE || email;
  console.log('[SMTP] sending', { host: process.env.SMTP_HOST, port: Number(process.env.SMTP_PORT || 2525), from: fromAddress, to: toAddress });
  ...
  console.log('[SMTP] accepted', { messageId: info.messageId, accepted: info.accepted, rejected: info.rejected });
  return info;
}
...
async function ensureSchema() {
  ...
  for (const statement of addColumnStatements) {
    try { await runQuery(statement); } catch (error) { if (error.code !== 'ER_DUP_FIELDNAME') throw error; }
  }

  try { await runQuery('ALTER TABLE Notes ADD COLUMN file_type VARCHAR(100)'); } catch (error) { if (error.code !== 'ER_DUP_FIELDNAME') throw error; }
...
// GET /Notes — includes like_count per note
app.get("/Notes", async (req, res, next) => {
  try {
    const { userId } = req.query;
    const baseQuery = `
      SELECT n.*, u.full_name AS seller_name,
             COUNT(nl.like_id) AS like_count
      FROM Notes n
      JOIN Users u ON u.user_id = n.user_id
      LEFT JOIN NoteLikes nl ON nl.note_id = n.note_id
      ${userId ? "WHERE n.user_id = ?" : ""}
      GROUP BY n.note_id
      ORDER BY n.created_at DESC
    `;
    const rows = userId
      ? await runQuery(baseQuery, [userId])
      : await runQuery(baseQuery);
    res.json(rows);
  } catch (err) { next(err); }
});
...
app.post("/Notes", async (req, res, next) => {
  try {
    ...
    const result = await runQuery(
      `INSERT INTO Notes (user_id, title, course_code, description, file_url, file_type) VALUES (?, ?, ?, ?, ?, ?)`,
...
// POST /api/notes/:id/like — toggle like/unlike
app.post("/api/notes/:id/like", async (req, res, next) => {
  try {
    ...
    const existing = await runQuery(
      "SELECT like_id FROM NoteLikes WHERE note_id = ? AND user_id = ? LIMIT 1",
      [noteId, userId]
    );

    if (existing.length > 0) {
      // Unlike
      await runQuery("DELETE FROM NoteLikes WHERE note_id = ? AND user_id = ?", [noteId, userId]);
      const [countRow] = await runQuery("SELECT COUNT(*) AS count FROM NoteLikes WHERE note_id = ?", [noteId]);
      return res.json({ liked: false, like_count: Number(countRow.count) });
    } else {
      // Like
      await runQuery("INSERT INTO NoteLikes (note_id, user_id) VALUES (?, ?)", [noteId, userId]);
      const [countRow] = await runQuery("SELECT COUNT(*) AS count FROM NoteLikes WHERE note_id = ?", [noteId]);
      return res.json({ liked: true, like_count: Number(countRow.count) });
    }
  } catch (err) { next(err); }
});

// GET /api/notes/likes?userId= — get all note_ids liked by a user
app.get("/api/notes/likes", async (req, res, next) => {
  try {
    const { userId } = req.query;
    if (!userId) return res.status(400).json({ error: "userId is required." });
    const rows = await runQuery(
      "SELECT note_id FROM NoteLikes WHERE user_id = ?",
      [userId]
    );
...
app.post('/api/register', async (req, res) => {
  ...
  const normalizedEmail = String(email).trim().toLowerCase();
  if (!isValidUwmEmail(normalizedEmail)) return res.status(400).json({ error: 'Email must be a valid UWM email address.' });
  if (String(password).length < 8) return res.status(400).json({ error: 'Password must be at least 8 characters.' });

  try {
    const existing = await runQuery('SELECT user_id, is_verified FROM Users WHERE email = ? LIMIT 1', [normalizedEmail]);
    ...
    if (existing.length > 0) {
      ...
      if (currentUser.is_verified) return res.status(409).json({ error: 'Email is already registered.' });
      await runQuery(`UPDATE Users SET verification_code_hash = ?, verification_expires_at = ? WHERE user_id = ?`, [verificationHash, verificationExpiresAt, currentUser.user_id]);
    ...
    const result = await runQuery(
      `INSERT INTO Users (full_name, email, password_hash, is_verified, verification_code_hash, verification_expires_at) VALUES (?, ?, ?, ?, ?, ?)`,
      [String(fullName).trim(), normalizedEmail, passwordHash, false, verificationHash, verificationExpiresAt]
    );
    await sendVerificationEmail(normalizedEmail, verificationCode);
    return res.status(201).json({ message: 'Account created. Check your email for the verification code.', userId: result.insertId });
  } catch (error) {
...
app.post('/api/verify-email', async (req, res) => {
  ...
  if (!isValidUwmEmail(normalizedEmail)) return res.status(400).json({ error: 'Email must be a valid UWM email address.' });
  if (!/^\d{6}$/.test(normalizedCode)) return res.status(400).json({ error: 'Verification code must be a 6-digit number.' });

  try {
    const users = await runQuery(`SELECT user_id, verification_code_hash, verification_expires_at, is_verified FROM Users WHERE email = ? LIMIT 1`, [normalizedEmail]);
    ...
    if (!user.verification_code_hash || !user.verification_expires_at) return res.status(400).json({ error: 'No active verification code. Register again to request a new code.' });
    const expiresAt = new Date(user.verification_expires_at);
    if (Number.isNaN(expiresAt.getTime()) || expiresAt < new Date()) return res.status(400).json({ error: 'Verification code expired. Register again to request a new code.' });
    const incomingHash = hashVerificationCode(normalizedCode);
    if (incomingHash !== user.verification_code_hash) return res.status(401).json({ error: 'Invalid verification code.' });
    await runQuery(`UPDATE Users SET is_verified = TRUE, verification_code_hash = NULL, verification_expires_at = NULL WHERE user_id = ?`, [user.user_id]);
...
app.post('/api/login', async (req, res) => {
  ...
  try {
    const users = await runQuery('SELECT user_id, full_name, email, password_hash, is_verified, profile_image_url, role FROM Users WHERE email = ? LIMIT 1', [normalizedEmail]);
    ...
    if (!user.is_verified) {
      ...
      const verificationHash      = hashVerificationCode(verificationCode);
      const verificationExpiresAt = getVerificationExpiryDate();
      await runQuery(`UPDATE Users SET verification_code_hash = ?, verification_expires_at = ? WHERE user_id = ?`, [verificationHash, verificationExpiresAt, user.user_id]);
      await sendVerificationEmail(normalizedEmail, verificationCode);
      return res.status(403).json({ error: 'Email not verified. A new verification code was sent to your UWM inbox.', requiresVerification: true });
    }

    if (user.is_suspended) return res.status(403).json({ error: 'Your account has been suspended. Please contact support.' });
    ...
    return res.status(200).json({
      ...
      user: { id: user.user_id, fullName: user.full_name, email: user.email, profile_image_url: user.profile_image_url, role: user.role },
    });
  } catch (error) {
...
app.post('/api/logout', async (_req, res) => res.status(200).json({ message: 'Logout successful.' }));

// ── Profile routes ────────────────────────────────────────────────────────────

app.get('/api/users/:id', async (req, res, next) => {
  try {
    const users = await runQuery('SELECT user_id, full_name, email, profile_image_url, role, created_at FROM Users WHERE user_id = ? LIMIT 1', [req.params.id]);
...
app.put('/api/users/:id/username', requireAuth, async (req, res, next) => {
  try {
    ...
    if (!isValidUsername(username)) {
      return res.status(400).json({ error: 'Username must be 2-30 letters or numbers only.' });
    }
...
app.put('/api/users/:id/password', requireAuth, async (req, res, next) => {
  try {
    ...
    const [existingUser] = await runQuery('SELECT user_id, password_hash FROM Users WHERE user_id = ? LIMIT 1', [targetUserId]);
    if (!existingUser) return res.status(404).json({ error: 'User not found.' });
    if (!verifyPassword(currentPassword, existingUser.password_hash)) {
      return res.status(401).json({ error: 'Current password is incorrect.' });
    }

    await runQuery('UPDATE Users SET password_hash = ? WHERE user_id = ?', [hashPassword(newPassword), targetUserId]);
...
app.delete('/api/listings/:id', async (req, res, next) => {
  try {
    await runQuery('DELETE FROM BookListings WHERE listing_id = ?', [req.params.id]);
    res.json({ success: true });
  } catch (err) { next(err); }
});
...
app.post("/api/images", upload.array("images", 6), async (req, res, next) => {
  try {
    ...
    if (prefix !== "Post_Pic") return res.status(400).json({ error: "Only Post_Pic uploads are supported here." });

    const numericListingId = Number(listingId);
    if (!Number.isInteger(numericListingId) || numericListingId <= 0) return res.status(400).json({ error: "A valid listingId is required." });
    ...
    if (existingCount + req.files.length > 6) return res.status(400).json({ error: "A listing can have at most 6 images." });
    ...
    for (let i = 0; i < req.files.length; i++) {
      ...
      if (orderNumber === 1) await runQuery("UPDATE BookListings SET image_url = ? WHERE listing_id = ?", [publicUrl, numericListingId]);
...
app.get("/api/upload-url", async (req, res, next) => {
  try {
    ...
    const command = new PutObjectCommand({ Bucket: process.env.R2_BUCKET_NAME, Key: key, ContentType: contentType, ChecksumAlgorithm: undefined });
    const uploadUrl = await getSignedUrl(r2Client, command, { expiresIn: 900, unhoistableHeaders: new Set(["x-amz-checksum-crc32"]) });
...
app.get("/api/notifications", async (req, res, next) => {
  try {
    const { userId } = req.query;
    if (!userId) return res.status(400).json({ error: "userId is required." });
    const rows = await runQuery(`SELECT * FROM Notifications WHERE user_id = ? ORDER BY created_at DESC LIMIT 50`, [userId]);
    res.json(rows);
  } catch (err) { next(err); }
});
...
app.get("/api/notifications/unread-count", async (req, res, next) => {
  try {
    const { userId } = req.query;
    if (!userId) return res.status(400).json({ error: "userId is required." });
    const rows = await runQuery(`SELECT COUNT(*) AS count FROM Notifications WHERE user_id = ? AND is_read = FALSE`, [userId]);
...
app.put("/api/notifications/:id/read", async (req, res, next) => {
  try {
    await runQuery(`UPDATE Notifications SET is_read = TRUE WHERE notification_id = ?`, [req.params.id]);
    res.json({ success: true });
  } catch (err) { next(err); }
});
...
app.get("/api/messages", async (req, res, next) => {
  try {
    ...
    if (!userId || !otherUserId || !listingId) return res.status(400).json({ error: "userId, otherUserId, and listingId are required." });
    const rows = await runQuery(
...
app.post("/api/messages", async (req, res, next) => {
  try {
    ...
    if (!senderId || !receiverId || !listingId || !content?.trim()) return res.status(400).json({ error: "senderId, receiverId, listingId, and content are required." });
...
app.use((err, req, res, next) => {
  console.error(err);
  if (String(err.message || "").startsWith("CORS blocked")) return res.status(403).json({ error: err.message });
  res.status(500).json({ error: "Server error" });
});

pool.getConnection()
  .then((conn) => { console.log('Successfully connected to the database'); conn.release(); return ensureSchema(); })
